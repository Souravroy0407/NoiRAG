"""
Evaluate Rule-Based Cleaner on Formatting Noise.
"""
import sys
...
from noirag.preprocessing.rule_based.cleaner import RuleBasedCleaner, clean_document_pages
from baseline.run_baseline import run_experiment, print_comparison_table, load_qa_pairs

def main():
    noisy_dir = PROJECT_ROOT / "data" / "noisy" / "formatting_25"
    cleaned_dir = PROJECT_ROOT / "data" / "cleaned" / "rule_based" / "formatting_25"
    
    # 1. Clean the noisy data
    print("1. Cleaning noisy data (formatting_25) with Rule-Based Cleaner...")
    ...
    # Limit to e.g. 50 QA pairs to make it quick, or testing all
    # We will test all for accurate comparison
    qa_pairs = load_qa_pairs()
    
    # Run experiment on Cleaned Data
    cleaned_result = run_experiment("rule_based_formatting_25", cleaned_dir, qa_pairs)
    
    # Run experiment on Noisy Data for comparison
    noisy_result = run_experiment("noisy_formatting_25", noisy_dir, qa_pairs)
